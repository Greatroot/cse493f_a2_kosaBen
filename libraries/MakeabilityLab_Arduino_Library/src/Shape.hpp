...
// const unsigned char P1CowboyBitmap2 [] PROGMEM = {
...
class PlayerBitMap : public Shape {
  protected:
    const unsigned char* _bitMapArray;

  public:
    PlayerBitMap(int x, int y, int width, int height, const unsigned char* bitMapArray) 
      : Shape(x, y, width, height), _bitMapArray(bitMapArray)
    {
      // purposefully empty
    }

    /**
     * @brief Draw the Player Bitmap
     * 
     * @param display 
     */
    void draw (Adafruit_SSD1306& display) override{
      // Draw BitMap takes in (xTop, yTop, BitMapPixelArray, width, height)
      display.drawBitmap(_x, _y,  _bitMapArray, _width, _height, WHITE);
